...
def findError(row, df_tripMain, toTime):
    try:
        ...
        df_subMain['DistC'] = df_subMain['Dist'].cumsum()
        t = row['Dist'] - toTime
        index1 = list((df_subMain['DistC']<t).index)
        random.shuffle(index1)
        df_subMain['Time'] = (df_subMain.index>index1[0]) * df_subMain['TimeDiff']
        df_subMain['Time'] = df_subMain['Time'].cumsum()
        df_subMain['DistC'] = (df_subMain.index>index1[0]) * df_subMain['Dist']
        df_subMain['DistC'] = df_subMain['DistC'].cumsum()
        df_subMain = df_subMain.loc[index1[0]:(df_subMain['DistC']<toTime).index[-1]+1]
...
def errorSummary(fromTime, timeFrame, df_tripStat, df_tripMain):
    try:
        df_subStat = df_tripStat[(df_tripStat['Dist']>fromTime)]
...
df_maskStat = {}
for dist in np.arange(0,10.1,0.5):
    df_maskStat[dist] = errorSummary(dist, 0.5, df_tripStat, df_tripMain)

# save the statistics for linear interpolation
outfile = open('/scratch/skp454/Trajectory/TaxiDataSet/data/20190709_TaxiLinearInterpolationDistShort.pkl','wb')
pickle.dump(df_maskStat,outfile)
outfile.close()

df_maskStat = {}
for dist in np.arange(0,61,3):
    df_maskStat[dist] = errorSummary(dist, 3, df_tripStat, df_tripMain)

# save the statistics for linear interpolation
outfile = open('/scratch/skp454/Trajectory/TaxiDataSet/data/20190709_TaxiLinearInterpolationDistLong.pkl','wb')
pickle.dump(df_maskStat,outfile)
outfile.close()
